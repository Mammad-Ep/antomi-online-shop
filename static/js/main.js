(function ($) {
	...
	$('.modal').addClass('invisible d-block');
	...
	$('.modal').removeClass('invisible d-block');
	...
	const min_price = document.getElementById('price-min');
	const max_price = document.getElementById('price-max');
	var min_value = Number(min_price.dataset.min);
	var max_value = Number(max_price.dataset.max);
	
	var values_price = []
	if (min_price.value == 0 && max_price.value == 0) {
		values_price = [min_value, max_value]
	} else {
		values_price = [min_price.value, max_price.value]
	}
